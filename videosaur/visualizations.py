...
def create_grid_frame(frames, grid_size=(2, 6), image_size=(518, 518), padding=2):
...
def create_grid_frame_rgb(frames, grid_size=(2, 6), image_size=(518, 518), padding=2):
    ...
    for index, frame in enumerate(frames):
        ...
        assert frame.shape[:2] == image_size, f"{frame.shape[:2]}, ....., {image_size}"
...
def mix_inputs_with_masks(inputs, outputs, softmasks=True):
    ...
    #print(f'mask shape: {masks_video.shape}')
    #resize masks to 224x224
    resizer = Resize(518, mode='bilinear')
